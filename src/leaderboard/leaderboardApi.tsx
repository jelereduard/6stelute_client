...
export const getLeaderboard: (
  token: string,
  idModul: any
) => Promise<LeaderboardProps[]> = async (token, idModul) => {
  ...
  return axios
    .get(`${leaderboardUrl}/${idModul}`, authConfig(token))
...
) => Promise<LeaderboardProps> = (token, leaderboard, networkStatus) => {
  return networkStatus
    ? axios
        .post(leaderboardUrl, leaderboard, authConfig(token))
        .then(response => {
...
export const updateLeaderboard: (
  ...
  if (networkStatus) {
    return axios
      .put(
        `${leaderboardUrl}/${leaderboard._id}`,
        leaderboard,
        authConfig(token)
      )
      .then(response => {
...
export const syncDataWithServer: (
  token: string
) => Promise<LeaderboardProps[]> = async token => {
...
async function getLeaderboardsLocal(
  keyFind: string
): Promise<LeaderboardProps[]> {
...
function setIfModifiedSinceHeader(
