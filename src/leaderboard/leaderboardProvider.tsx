...
export const ProductProvider: React.FC<ProductProviderProps> = ({
  ...
  const [settingsSavedOffline, setSettingsSavedOffline] =
    useState<boolean>(false)
  ...
  const fetchLeaderboard = useCallback<FetchLeaderboardFn>(
    fetchModuleCallback,
    [token, idModul]
  )
  const value = {
  ...
  async function saveProductCallback(product: ProductProps) {
    try {
      ...
      const savedProduct = await (!product._id
        ? createProduct(token, product, connectedNetworkStatus)
